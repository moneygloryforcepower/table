...
function drawCharts() {
  ...
  if (barChart) {
    ...
    barChart = new Chart(barCtx, {
      type: 'bar',
      data: { labels: groupLabels, datasets: [{ label: 'Присутствовало студентов', data: groupValues, backgroundColor: 'rgba(54, 162, 235, 0.7)' }] },
      options: { responsive: false, plugins: { title: { display: true, text: 'Посещаемость по группам' } }, scales: { y: { beginAtZero: true, precision: 0 } } }
    });
  }
  ...
  if (pieChart) {
    ...
    pieChart = new Chart(pieCtx, {
      type: 'pie',
      data: { labels: groupLabels, datasets: [{ data: groupValues, backgroundColor: ['#ff6384','#36a2eb','#ffcd56'] }] },
      options: { responsive: false, plugins: { title: { display: true, text: 'Доля посещаемости групп' } } }
    });
  }
  ...
  if (lineChart) {
    ...
    lineChart = new Chart(lineCtx, {
      type: 'line',
      data: { labels: weekLabels, datasets: [{ label: 'Посещаемость по дням', data: weekValues, borderColor: 'green', backgroundColor: 'rgba(0, 255, 0, 0.2)', tension: 0.3 }] },
      options: { responsive: false, plugins: { title: { display: true, text: 'Посещаемость по дням недели' } }, scales: { y: { beginAtZero: true, precision: 0 } } }
    });
  }
}
